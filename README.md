...
The reason "just use max torque everywhere at once" doesn't work is because the joints are physically coupled, and 2 things are going on:
...
python experiments/whip_optimize.py     # run the optimization, saves saved/best_swing.npy
python experiments/whip_watch.py         # replay the trained swing in the viewer
python experiments/benchmark.py          # trained vs the two baselines, prints impact speeds
```

The watcher takes an argument for what to watch:

```
python experiments/whip_watch.py trained
python experiments/whip_watch.py foldsnap
python experiments/whip_watch.py allatonce
```
...
experiments/whip_optimize.py   CMA-ES optimizer
experiments/whip_watch.py      viewer playback
experiments/benchmark.py       trained vs baselines
saved/best_swing.npy           the trained swing (so you can watch without retraining)
```

## Limitations

- **Narrow speed margin.** The trained swing only beats the best brute force baseline by about 3% on speed (9.76 vs 9.43). I believe that the idea is strong, but the sim lacks the proper setup to prove the margin could be much larger under other scenarios.
...
That said, the speed margin was only a few percent and I believe that is because the sim wasn't as strong as I hoped, so hopeful I can build a much more thourough one in the future. Also, the result is tied closely to this specific setup, specifically the start pose, the block position relative to the fist, and the arm's reach. It's hard to say how well any of this generalizes to a different geometry. Still, the effect is real and shows up in the high fidelity physics environment.

## AI usage

I used an AI assistant (Claude) on this project to work decide on the open-loop CMA-ES alorithm, pick hyperparameters (sigma0 = 0.5, popsize = 16, horizon), write and debug the code, and design the reward. The body XML is AI-generated and then tuned by hand. It also helped write and organize this README to structure the results and limitations clearly.
